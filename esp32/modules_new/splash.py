...
import ugfx, time, badge, machine, gc, sys, version, orientation, term_menu
import helpers.term as term, helpers.system as system, helpers.scheduler as scheduler, helpers.graphics as graphics, helpers.wifi as wifi, helpers.rtc as rtc
import tasks.powermanagement as pm, tasks.otacheck as otac, tasks.services as services
...
class Splash:
	...
	def home(self):
		self.redraw = True
		self.touch_flush()
		scheduler.delete(self.task_main)
		scheduler.new(1, self.task_main, True)
	...
	def menu(self, title, items, callbacks):
		...
		scheduler.delete(self.menu_task)
		scheduler.new(1, self.menu_task, True)
	...
	def on_sleep(self, idleTime):
		...
		self.leds_off()
		ugfx.flush()
		system.sleep(idleTime) #Seems double (pm does this as well) but needed for uart menu
